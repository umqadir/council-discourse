...
All costs are measured OpenRouter billing (exact generation cost where OpenRouter
returned it, otherwise its own per-token model pricing applied to measured tokens).
Per-meeting figures are naming plus chaptering, averaged over the two benchmark
meetings. Monthly figures assume 20 meetings per month.

## Decision table

| Config | naming same-person (transp / stated) | chaptering F1@30 (transp / stated) | Clears gates? | $/meeting | $/mo (20 mtgs) |
| --- | --- | --- | --- | --- | --- |
| GLM-5.2 both passes (current baseline) | 87.9% / 98.6% | 75.5% / 91.7% | yes | $0.282 | $5.64 |
| DeepSeek V4 Pro both passes | 87.9% / 97.3% | 69.0% / 84.1% | no (chaptering fails both) | $0.107 | $2.15 |
| Combined single pass, GLM-5.2 | 86.3% / 100.0% | 70.6% / 75.4% | no (chaptering fails both; naming fails transp) | $0.458 | $9.15 |
| Recommended: DeepSeek V4 Pro naming + GLM-5.2 chaptering | 87.9% / 97.3% | 75.5% / 91.7% | yes | $0.188 | $3.76 |
...
passes, so there is no quality regression) and cuts the LLM line by a third, from
$5.64 to $3.76 per month, under the ~$4 target.
...
calls. The one place a genuine repeated prefix exists is chaptering's conditional
coarse-retry, which resends its own prompt — but that retry only fires when the
first draft undershoots the chapter-count floor, so it is not a reliable saving.
...
- Cost: the combined pass cost *more*, not less — $0.572 (transportation) and
  $0.343 (stated), averaging $0.458 per meeting versus $0.282 for the two-pass
  baseline. Emitting speaker segments and chapters in one response roughly doubles
  completion tokens, and completion is GLM's expensive side ($3.00 per million
  versus $0.93 for input). The hoped-for saving was on input tokens (one transcript
  read instead of two), but naming never sent the full transcript to begin with, so
  there was little input to save, and the extra output swamped it.
...
- Naming: ties both gates exactly. Same-person 87.9% (transportation) and 97.3%
  (stated), at about a third of GLM's naming cost ($0.074 vs $0.160 transportation;
  $0.053 vs $0.156 stated).
...
already passes) and brings the LLM line to $0.188 per meeting, about $3.76 per
month at 20 meetings — a 33% reduction from the $5.64 baseline, under the ~$4
target. The change is a per-stage model override in the pipeline; no prompt or
schema restructure is needed.

Do not pursue the combined single pass (worse quality, higher cost) or prompt
caching (no meaningful shared prefix in the current architecture). If cost needs to
drop further later, the open question worth answering is Z.AI-direct GLM pricing
and native caching for the chaptering pass, which was not verifiable in this
environment.

## Spend

This round used about $1.4 of the $5 OpenRouter budget (DeepSeek V4 Pro full matrix
plus the GLM combined-pass runs and cache probes).
