...
Verification runs in small batches and applies model corrections only when Google returns actual search sources. Council-member aliases are normalized before verification to avoid unnecessary searches. ASR rejects speech with missing diarization rather than publishing it.
